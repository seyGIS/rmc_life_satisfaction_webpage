...
author: "SEYDINA ISSA LAYE FALL" # Remember to add your name!
date: "16-10-2024"
theme: united
---
...
cantril %>%
  ...
  mutate(
    ...
    Relative_change = round((` 2023` - ` 2011`) / ` 2011` * 100, 2)) %>%
  reactable()
...
# Create line chart
simple_chart <- cantril %>%
  filter(Country %in% unique(Country)[1:4]) %>%
  select(Year, `Cantril ladder score`, Country)
ggplotly( ggplot(data = simple_chart, aes(x = Year, y = `Cantril ladder score`, group = Country, color = Country)) +
  geom_line() +
  scale_y_continuous(limits = c(0, 10)) +
  labs(title = "Cantril Score Evolution in 4 Countries", x = "Year", y = "Cantril Ladder Score") +
  theme_minimal()
)
